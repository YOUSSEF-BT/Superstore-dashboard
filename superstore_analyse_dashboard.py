...
# ============================================
# COMPLETE SUPERSETORE ANALYSIS - FIXED VERSION
# ============================================
...
print("📥 Télécharge le fichier 'Sample - Superstore.csv'")
...
print(f"Colonnes ajoutées: {list(df.columns[-6:])}")

# ========== ÉTAPE 3: CONFIGURATION SQL CORRECTE ==========
print("\n⚙️ CONFIGURATION SQL...")
!pip install pandasql -q
from pandasql import sqldf

# FONCTION CORRIGÉE - Version qui marche
def run_sql(query):
    """Exécute une requête SQL sur le DataFrame df"""
    return sqldf(query, globals())

# Test simple
test_query = "SELECT COUNT(*) as total_rows FROM df"
test_result = run_sql(test_query)
print(f"✅ SQL configuré - Test réussi: {test_result['total_rows'][0]} lignes")

# ========== ÉTAPE 4: REQUÊTE 1 - PERFORMANCE MENSUELLE ==========
print("\n📈 REQUÊTE 1: Performance mensuelle")
...
print(f"\n🔝 Meilleur mois (CA): {result1.loc[result1['Monthly_Sales'].idxmax(), 'Month']}")
print(f"📉 Moins bon mois (marge): {result1.loc[result1['Avg_Margin_Percent'].idxmin(), 'Month']} ({result1['Avg_Margin_Percent'].min()}%)")

# ========== ÉTAPE 5: REQUÊTE 2 - TOP CLIENTS ==========
print("\n👥 REQUÊTE 2: Analyse des clients")
...
# ========== ÉTAPE 6: REQUÊTE 3 - PRODUITS PERFORMANTS ==========
print("\n📦 REQUÊTE 3: Top produits")
...
# ========== REQUÊTE 4 CORRIGÉE ==========
print("\n🌎 REQUÊTE 4: Performance géographique")
query4 = """
SELECT
    Region,
    State,
...
# ========== REQUÊTE 5 CORRIGÉE ==========
print("\n📅 REQUÊTE 5: Performance par jour de semaine")
query5 = """
SELECT
    "Order Weekday",
...
print("📊 PRÉPARATION DES DONNÉES POUR TABLEAU...")

# 1. Créer une vue mensuelle (pour les séries temporelles)
...
with pd.ExcelWriter('superstore_tableau.xlsx') as writer:
    ...
    category_sales.to_excel(writer, sheet_name='Par_Catégorie', index=False)
    geo_sales.to_excel(writer, sheet_name='Par_Région', index=False)
    top_products.to_excel(writer, sheet_name='Top_Produits', index=False)

print("✅ Fichier Excel créé avec 5 feuilles :")
print("1. Données_Brutes - Toutes les données nettoyées")
print("2. Ventes_Mensuelles - Pour graphiques temporels")
print("3. Par_Catégorie - Pour graphiques catégoriels")
print("4. Par_Région - Pour cartes géographiques")
...
# ============================================
# DASHBOARD SUPERSETORE - VERSION AVEC CARTE USA CORRIGÉE
# ============================================
...
print("🚀 INITIALISATION DU DASHBOARD SUPERSETORE...")
print("="*60)
...
# Définir les sous-catégories et produits par catégorie
sub_categories = {
...
# Dictionnaire de correspondance État -> Code
state_to_code = {
...
# États disponibles avec leurs codes
...
# Générer les sous-catégories et produits en fonction des catégories
sub_cats = []
prod_names = []
...
print(f"\n📊 KPI CALCULÉS :")
print(f"   • CA Total : ${total_sales:,.0f}")
print(f"   • Profit Total : ${total_profit:,.0f}")
print(f"   • Marge Moyenne : {avg_margin:.1f}%")
print(f"   • Commandes : {total_orders:,}")

# === 3. PRÉPARATION DES DONNÉES POUR LES GRAPHIQUES ===
print("\n📈 Préparation des données pour les graphiques...")

# Données pour le graphique 1 (évolution mensuelle)
monthly_sales = df.groupby('Order Month').agg({
    'Sales': 'sum',
    'Profit': 'sum'
}).reset_index()

# Données pour le graphique 2 (catégories)
category_sales = df.groupby(['Category', 'Sub-Category']).agg({
    'Sales': 'sum',
    'Profit': 'sum'
}).reset_index().sort_values('Sales', ascending=False)

# Données pour le graphique 3 (carte géographique - CORRIGÉ)
...
print(f"\n✅ Données carte préparées : {len(state_sales)} états")
print(f"   Exemple : {state_sales[['State', 'State_Code', 'Sales']].head().to_string(index=False)}")

# Données pour le graphique 4 (top produits)
...
print("\n" + "="*60)
print("📊 AFFICHAGE DES GRAPHIQUES INDIVIDUELS")
print("="*60)
...
fig2 = px.sunburst(
    ...
    values='Sales',
    title='📊 Répartition des Ventes (Sunburst)',
    color='Sales',
...
# Graphique 3 : Carte géographique USA - VERSION CORRIGÉE
print("\n✅ Graphique 3 : Carte USA corrigée")
print(f"   États à afficher : {len(state_sales)} états")

# Créer la carte avec les codes d'état
fig3 = go.Figure(data=go.Choropleth(
    locations=state_sales['State_Code'],  # Utiliser les codes d'état
    locationmode='USA-states',  # Mode USA
    z=state_sales['Sales'].astype(float),  # Valeurs numériques
...
fig3.update_layout(
    title_text='🌍 Carte des Ventes par État (USA)',
    geo=dict(
        scope='usa',
...
# Ajouter les noms des états manuellement
for i, row in state_sales.iterrows():
    fig3.add_trace(go.Scattergeo(
        lon=[None],  # Nous n'avons pas de coordonnées exactes
        lat=[None],
        text=[row['State_Code']],
        mode='text',
        showlegend=False,
...
# === 5. CRÉATION DU DASHBOARD UNIFIÉ AVEC CARTE ===
print("\n" + "="*60)
print("📊 DASHBOARD COMPLET AVEC CARTE USA")
print("="*60)

# Créer une figure avec subplots (simplifiée pour éviter les conflits)
...
# Graphique 4 : Carte USA - VERSION SIMPLIFIÉE
fig_dashboard.add_trace(
    go.Choropleth(
        ...
        colorbar_title="Ventes ($)",
        showscale=True,
        name='Ventes par État',
        hoverinfo='location+z'
    ),
    row=2, col=2
)

# Configuration de la géographie pour la carte
...
fig_dashboard.update_layout(
    height=800,
    title_text=f"📊 Dashboard Superstore - {datetime.now().strftime('%d/%m/%Y')}",
    title_font_size=20,
    title_x=0.5,
    showlegend=True,
...
fig_dashboard.update_xaxes(title_text="Catégorie", row=2, col=1)
...
print(f"   • Valeur moyenne par commande : ${(total_sales/total_orders):,.0f}")
print(f"   • Période analysée : {df['Order Date'].min().date()} au {df['Order Date'].max().date()}")
...
try:
    ...
    print("   • superstore_data_complete.csv - Données complètes")
    print("   • superstore_summary.csv - Résumé")
    print("   • superstore_state_sales.csv - Données par état")
...
print("="*60)
print("\n🎯 RÉCAPITULATIF :")
print("   1. ✅ 4 graphiques individuels")
print("   2. ✅ 1 dashboard avec carte USA fonctionnelle")
print("   3. ✅ Codes d'état utilisés (CA, TX, NY, etc.)")
print("   4. ✅ Données exportées en CSV")